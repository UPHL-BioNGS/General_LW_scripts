...
for folder in folders:
    bashCommand = "icav2 projectdata list --data-type FILE --parent-folder %s --file-name bsshinput.json" % (folder)
    tmp= icav_out(bashCommand)
...
if len(ica_target) > 1:
    for i in ica_target:
        bashCommand = "icav2 projectdata list --data-type FILE --parent-folder %s --file-name bsshoutput.json" % (folders[i])
        try:
            tmp= icav_out(bashCommand)
        except:
            del ica_target[i]
...
bashCommand = "icav2 projectdata list  --parent-folder " +  ica_target + "output/fastqs/Samples/"
tmp= icav_out(bashCommand)
...
ica_names=[]
for i in ica_lanes:
    bashCommand = "icav2 projectdata list  --parent-folder  " +  ica_target + "output/fastqs/Samples/Lane_1/"
    tmp= icav_out(bashCommand)
    for j in tmp:
