...
# LIMS_TEST_ID,Genomic Coverage,AMR Gene symbol,AMR Name of closest sequence,AMR % Coverage of reference sequence,AMR % Identity to reference sequence,Virulence Gene symbol,Virulence Name of closest sequence,Virulence % Coverage of reference sequence,Virulence % Identity to reference sequence,SerotypeFInder (E. coli),SeqSero Organism (Salmonella),Run Name,Genomic Coverage,Passed QC,Shiga toxin genes identified,Allelle Code,Rep Code,PNUSA Accession,BioSample Accession,Date Uploaded to CDC,Date Uploaded to NCBI/SRA,Passed PulseNet QC
pulsenet_labware_cols = ['LIMS_TEST_ID','Genomic Coverage','AMR Gene symbol','AMR Name of closest sequence','AMR % Coverage of reference sequence','AMR % Identity to reference sequence','Virulence Gene symbol','Virulence Name of closest sequence','Virulence % Coverage of reference sequence','Virulence % Identity to reference sequence','SerotypeFInder (E. coli)','SeqSero Organism (Salmonella)','Run Name','Genomic Coverage','Passed QC','Shiga toxin genes identified','Allelle Code','Rep Code','PNUSA Accession','BioSample Accession','Date Uploaded to CDC','Date Uploaded to NCBI/SRA','Passed PulseNet QC']
...
for sample in sample_list:
    ...
    result.to_csv('labware8/arln_'     + sample + '_results.txt', columns = arln_labware_cols ,    index=False)
    result.to_csv('labware8/pulsenet_' + sample + '_results.txt', columns = pulsenet_labware_cols, index=False)
    result.to_csv('labware8/altngs_'   + sample + '_results.txt', columns = alt_labware_cols,      index=False)