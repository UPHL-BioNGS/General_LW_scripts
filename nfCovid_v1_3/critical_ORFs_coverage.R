...
vec_S<-vector(); vec_E<-vector(); vec_M<-vector(); vec_N<-vector(); 

Coverage<-vector();
sampl<-vector();
n <-nrow(Samples); 
vec_nsp14<-vector();vec_ORF10<-vector();vec_ORF3a<-vector();vec_ORF6<-vector();vec_ORF7a<-vector();vec_ORF7b<-vector();vec_ORF8<-vector() 
...
# index
  ...
  # location extracted from https://www.ncbi.nlm.nih.gov/nuccore/NC_045512.2 
...
# save results
  ...
  critical_ORFs_coverage <-cbind(nsp14_coverage,ORF10_coverage[,2],ORF3a_coverage[,2],ORF6_coverage[,2],ORF7a_coverage[,2],ORF7b_coverage[,2],ORF8_coverage[,2])
    colnames(critical_ORFs_coverage)<-c("Sample","nsp14 meanDepth","ORF10 meanDepth","ORF3a meanDepth","ORF6 meanDepth","ORF7a","ORF7b meanDepth","ORF8 meanDepth");
   path2<-paste("/Volumes/NGS/Analysis/covidseq",runName,"critical_ORF-meandepth_coverage.csv",sep = "/");
  write.csv(critical_ORFs_coverage,path2)
